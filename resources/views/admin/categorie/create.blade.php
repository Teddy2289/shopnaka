...
@section('content')
    ...
    <div class="card-block caption-breadcrumb">
        ...
        <div class="page-header-breadcrumb">
            <ul class="breadcrumb-title">
                ...
                <li class="breadcrumb-item"><a href="#!">{{__('Nouveau catégories')}}</a>
                </li>
            </ul>
        </div>
    </div>
    <div class="card">
        <div class="card-header">
            <h5 class="mt-2">{{__('Nouveau catégories')}}</h5>
    ...
    <div class="card">
        <div class="card-block">
            <form id="main" method="post" action="{{route('categorie.store')}}"  novalidate="novalidate" >
                {{ csrf_field() }}
                <div class="form-group row has-error">
                    <label class="col-sm-2 col-form-label">Catégorie</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control  {{ $errors->has('name') ? ' is-invalid' : '' }} input-danger" value="{{ old('name') }}" name="name" id="name" placeholder="catégorie name" required>
                        @if($errors->has('name'))
